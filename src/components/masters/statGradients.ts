...
// the same indigo-to-blue gradient used for Add-X buttons and table header
// bands (see master.css's --brand-gradient) — instead of a per-card accent
// gradient, per explicit product decision — so this deliberately ignores
...
// this sets a JS/inline-style string, not a stylesheet rule — keep the two
// hex stops in sync with master.css's --brand-gradient if either changes.
const STAT_CARD_BRAND_GRADIENT = 'linear-gradient(135deg, #6366f1, #4f46e5)';
