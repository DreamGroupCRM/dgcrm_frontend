...
const PaginationFooter: React.FC<PaginationFooterProps> = ({
  ...
  <div className="master-pagination" style={{ borderTop: `1px solid ${t.divider}` }}>
    ...
    <div className="flex items-center gap-1">
      <button onClick={() => setPage(Math.max(1, safePage - 1))} disabled={safePage === 1} className="master-page-btn"
      ...
      {pageBtns().map((pg) => (
        <button key={pg} onClick={() => setPage(pg)} className="master-page-btn"
          style={{ border: `1px solid ${pg === safePage ? '#0284c7' : t.surfaceBorder}`, background: pg === safePage ? '#0284c7' : t.btnSecondaryBg, color: pg === safePage ? '#fff' : t.textPrimary, fontWeight: pg === safePage ? 700 : 400 }}>
          {pg}
        </button>
      ))}
      <button onClick={() => setPage(Math.min(totalPages, safePage + 1))} disabled={safePage >= totalPages} className="master-page-btn"
