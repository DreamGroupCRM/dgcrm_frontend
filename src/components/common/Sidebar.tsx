...
const NavItemComponent: React.FC<{
    ...
    return (
      <div>
        ...
        >
          ...
          {!collapsed && (
            <>
              <span className="flex-1 text-left font-medium" style={{ fontSize: 12 }}>{item.label}</span>
  ...
  return (
    ...
    >
      <span className="flex-shrink-0" style={{ fontSize: 16 }}>{item.icon}</span>
      {!collapsed && (
        <span className="font-medium" style={{ fontFamily: t.fontFamily, fontSize: 12 }}>
          {item.label}
        </span>
      )}
    </NavLink>
      ...
      {!collapsed && (
        <div className="px-4 py-2.5 flex-shrink-0">
          <span className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full font-semibold"
            style={{ background: isDark ? '#141414' : '#efebe9', color: isDark ? '#a3a3a3' : '#0000FF', fontFamily: t.fontFamily, fontSize: 9.5 }}>
      ...
      {showCollapseToggle && (
        <div className="px-3 pb-4 pt-2 flex-shrink-0" style={{ borderTop: `1px solid ${t.sidebarBorder}` }}>
          <button
            ...
            style={{ background: 'transparent', color: t.textPrimary, border: 'none', cursor: 'pointer', fontFamily: t.fontFamily, fontSize: 12 }}
