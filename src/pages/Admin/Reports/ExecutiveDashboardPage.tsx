...
const CHART_COLORS = ['#0000FF', '#16a34a', '#ea580c', '#7c3aed', '#dc2626', '#0891b2', '#db2777'];
...
const ACTION_META: Record<string, { label: string; color: string; icon: React.ElementType }> = {
  ...
  login_success: { label: 'logged in', color: '#0000FF', icon: MdLogin },
...
const PatternCard: React.FC<{ t: Theme; isDark: boolean; pattern: DetectedPattern }> = ({ t, isDark, pattern }) => {
  ...
  const color = isWarning ? '#dc2626' : '#0000FF';
  return (
  ...
  return (
    <div className="exec-dash-page" style={{ fontFamily: t.fontFamily }} ref={exportRef}>
          ...
          <div className="exec-dash-preset-group flex items-center gap-1.5 flex-wrap" style={{ marginRight: 8 }}>
            ...
            {(Object.keys(PRESET_LABELS) as PresetKey[]).map((k) => (
              <button key={k} type="button" onClick={() => setPreset(k)}
                ...
                style={{
                  background: preset === k ? tintColor('#0000FF') : t.insetBg,
                  color: preset === k ? '#fff' : t.textSecondary,
                  border: `1px solid ${preset === k ? tintColor('#0000FF') : t.surfaceBorder}`, cursor: 'pointer',
                }}>
                {PRESET_LABELS[k]}
              </button>
            ))}
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3" style={{ marginBottom: 18 }}>
        <StatCard label="Total Customers" value={loading ? '—' : kpis!.total_customers} icon={MdPeople} color="#0000FF" bg="" loading={loading}
          ...
          {loading ? <Skeleton t={t} /> : !data || data.sales_trend.length === 0 ? (
            ...
            <ResponsiveContainer width="100%" height={280}>
              <LineChart data={data.sales_trend} margin={{ top: 8, right: 12, left: 0, bottom: 0 }}>
                ...
                />
                <Line type="monotone" dataKey="booking_value" stroke="#0000FF" strokeWidth={2.5} dot={{ r: 3 }} name="booking_value" />
              </LineChart>
            </ResponsiveContainer>
          )}
        </SectionCard>
              ...
              {data.customer_overview.monthly_new_customers.length === 0 ? (
                ...
                <ResponsiveContainer width="100%" height={200}>
                  <BarChart data={data.customer_overview.monthly_new_customers} margin={{ top: 4, right: 8, left: 0, bottom: 0 }}>
                    ...
                    <Tooltip contentStyle={{ background: t.surfaceBg, border: `1px solid ${t.surfaceBorder}`, borderRadius: 10, fontSize: 12 }} />
                    <Bar dataKey="count" fill="#0000FF" radius={[4, 4, 0, 0]} name="New Customers" />
                  </BarChart>
                </ResponsiveContainer>
              )}
            </>
