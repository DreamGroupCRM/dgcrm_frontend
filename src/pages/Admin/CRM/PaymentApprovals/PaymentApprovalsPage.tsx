    ...
    <div className="pa-page" style={{ fontFamily: t.fontFamily, ...cssVars }}>
      ...
      <div className="pa-stat-grid grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 mb-5">
        ...
        <StatCard label="Total Approved This Month" value={stats?.approved_this_month ?? 0} icon={MdEventAvailable} color="#16a34a" bg="" loading={!stats}
          ...
          surfaceBg={t.surfaceBg} surfaceBorder={t.surfaceBorder} textPrimary={t.textPrimary} textSecondary={t.textSecondary} />
        <StatCard label="Total Approved Today" value={stats?.approved_today ?? 0} icon={MdToday} color="#4e342e" bg="" loading={!stats}
          onClick={() => navigate(ROUTES.ADMIN.PAYMENT_RECEIVED)}
          surfaceBg={t.surfaceBg} surfaceBorder={t.surfaceBorder} textPrimary={t.textPrimary} textSecondary={t.textSecondary} />
      </div>
              ...
              {loading ? (
                ...
                rows.map((r) => (
                  <tr key={r.id} style={{ borderTop: `1px solid ${t.divider}` }}>
                    ...
                    <td style={{ padding: '12px 14px' }}>
                      <span className="inline-flex items-center px-2 py-1 rounded-md font-semibold" style={{ background: isDark ? 'rgba(37,99,235,0.15)' : '#dbeafe', color: '#4e342e', fontSize: 10.5, whiteSpace: 'nowrap' }}>
                        {r.mode_of_payment || '—'}
                      </span>
                    </td>
                    <td style={{ padding: '12px 14px' }}>
                ...
                <>
                  ...
                  <div className="pa-modal-footer flex items-center justify-end gap-2.5 flex-wrap" style={{ borderTop: `1px solid ${t.divider}`, paddingTop: 16 }}>
                    ...
                    <button type="button" onClick={() => viewModal.data && setReceiptPreview(viewModal.data)}
                      className="flex items-center gap-1.5 px-4 py-2 rounded-xl text-sm font-semibold"
                      style={{ background: isDark ? 'rgba(37,99,235,0.15)' : '#dbeafe', border: 'none', color: '#4e342e', cursor: 'pointer' }}>
