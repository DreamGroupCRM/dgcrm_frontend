...
// Payment" checkbox+result-box UI. The stat box above keeps that exact
// same look/behaviour (pick a date range, press OK, see the total for
// that range — GET /payments/upcoming-amount). New here: a table below it
// that stays empty until that same OK click, then lists every individual
// upcoming installment in the range (GET /payments/upcoming-list-
// detailed) so an admin can see, per customer, which date and how much is
// coming and call the assigned employee to chase it up — same tabular
// convention as the Payment Dues table (consolidated Contact and
// Building/Wing/Flat columns), plus 3 client-side search fields over the
// fetched list (Customer ID/Name, Employee Name, Building Name).
import React, { useEffect, useMemo, useState } from 'react';
import { toast } from 'react-toastify';
import { MdUpcoming, MdRefresh, MdSearch, MdEvent, MdAccountBalanceWallet } from 'react-icons/md';
...
const PaymentUpcomingPage: React.FC = () => {
  ...
  const filteredRows = useMemo(() => {
    ...
    return listRows.filter((r) => {
      if (c && !(r.customer_name.toLowerCase().includes(c) || r.customer_code.toLowerCase().includes(c))) return false;
      if (e && !(r.assigned_employee_name || '').toLowerCase().includes(e)) return false;
  ...
  return (
    <div className="payment-upcoming-page" style={{ fontFamily: t.fontFamily, ...cssVars }}>
      ...
      {/* ── Date range + OK + stat box — same look/behaviour as the removed
          Payment Dues "Show Upcoming Payment" checkbox. ─────────────────── */}
      ...
      {/* ── 3 search fields — Customer ID/Name, Employee Name, Building
          Name. Only meaningful once a range has been applied. ──────────── */}
      <div className="payment-upcoming-search-card rounded-2xl mb-5 p-4" style={{ background: t.surfaceBg, border: `1px solid ${t.surfaceBorder}` }}>
        <div className="payment-upcoming-search-row grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
          <div className="payment-upcoming-search-field relative">
            ...
            <input type="text" placeholder="Search by Customer ID or Name" value={searchCustomer} onChange={(e) => setSearchCustomer(e.target.value)}
              style={{ ...fieldInputStyle, paddingLeft: 30 }} disabled={!applied} />
          </div>
          <div className="payment-upcoming-search-field relative">
            <MdSearch size={15} style={{ position: 'absolute', left: 10, top: 11, color: t.textSecondary, pointerEvents: 'none' }} />
            <input type="text" placeholder="Search by Employee Name" value={searchEmployee} onChange={(e) => setSearchEmployee(e.target.value)}
              style={{ ...fieldInputStyle, paddingLeft: 30 }} disabled={!applied} />
          </div>
          <div className="payment-upcoming-search-field relative">
            <MdSearch size={15} style={{ position: 'absolute', left: 10, top: 11, color: t.textSecondary, pointerEvents: 'none' }} />
      ...
      <div className="payment-upcoming-table-card rounded-2xl" style={{ background: t.surfaceBg, border: `1px solid ${t.surfaceBorder}` }}>
        <div className="payment-upcoming-table-scroll" style={{ overflowX: 'auto' }}>
          <table className="payment-upcoming-table" style={{ width: '100%', borderCollapse: 'collapse', minWidth: 1100 }}>
            <thead>
              <tr className="master-table-header-gradient" style={{ background: t.tableHeaderBg }}>
                {['Customer', 'Assigned Employee', 'Building / Wing / Flat', 'Contact (Email / Mobile)', 'Payment For', 'Due Date', 'Amount', 'Status'].map((h) => (
            ...
            <tbody>
              {!applied ? (
                ...
                <tr><td colSpan={8} style={{ padding: 28, textAlign: 'center', color: t.textSecondary }}>Loading upcoming payments...</td></tr>
