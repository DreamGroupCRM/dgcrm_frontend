      ...
      {displayName ? (
        <div className="flex items-center gap-2 px-3 py-2 rounded-xl" style={{ background: t.insetBg, border: `1px solid ${t.inputBorder}` }}>
          {previewUrl ? (
            ...
            <MdInsertDriveFile size={16} style={{ color: '#4e342e', flexShrink: 0 }} />
          )}
  ...
  return (
    <div className="rounded-xl p-4 text-center" style={{ border: `1.5px dashed ${t.inputBorder}`, background: t.inputBg }}>
      ...
      {previewUrl ? (
        ...
        <MdCloudUpload size={26} style={{ color: '#4e342e', margin: '0 auto 6px' }} />
      )}
      {displayName ? (
        ...
        <button type="button" disabled={isView} onClick={() => inputRef.current?.click()}
          style={{ background: 'transparent', border: 'none', cursor: isView ? 'not-allowed' : 'pointer', color: '#4e342e', fontSize: 11, fontWeight: 700, fontFamily: t.fontFamily }}>
          Upload {label}
        </button>
      )}
...
const RadioOption: React.FC<{ t: Theme; label: string; selected: boolean; onSelect: () => void; disabled?: boolean }> = ({ t, label, selected, onSelect, disabled }) => (
  <button type="button" disabled={disabled} onClick={onSelect}
    ...
    {selected ? <MdRadioButtonChecked size={18} style={{ color: '#4e342e' }} /> : <MdRadioButtonUnchecked size={18} style={{ color: t.textSecondary }} />}
...
const AmountField: React.FC<{ t: Theme; isView?: boolean; disabled?: boolean; placeholder: string; value: string; onChange: (v: string) => void }> = ({ t, isView, disabled, placeholder, value, onChange }) => {
  ...
  return (
    <div className={`flex items-center gap-1.5 ${fieldClassName(!!isView || !!disabled)}`} style={{ padding: '0 10px' }}>
      ...
      {compact && <span style={{ color: '#4e342e', fontWeight: 700, fontSize: 10, flexShrink: 0, whiteSpace: 'nowrap' }}>{compact}</span>}
    </div>
  );
};
      ...
      <AccordionSection theme={t} icon={<MdPerson size={16} />} title="Customer Details" gradient="var(--grad-green)"
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-3 mb-4">
          ...
          <Field t={t} label="Upload Aadhar Card Photo" required error={errorFor('aadharPhoto')} fieldRef={setFieldRef('aadharPhoto') as React.Ref<HTMLDivElement>}>
            ...
            {ocrRunning === 'aadhar' && <p style={{ fontSize: 10, color: '#4e342e', margin: '4px 0 0' }}>Reading Aadhar number from photo...</p>}
          </Field>
          ...
          <Field t={t} label="Upload Pancard Photo" required error={errorFor('pancardPhoto')} fieldRef={setFieldRef('pancardPhoto') as React.Ref<HTMLDivElement>}>
            ...
            {ocrRunning === 'pancard' && <p style={{ fontSize: 10, color: '#4e342e', margin: '4px 0 0' }}>Reading PAN number from photo...</p>}
          </Field>
            ...
            <div className="flex items-center gap-2">
              ...
              {age && (
                <div className="rounded-xl px-2 py-2 flex-shrink-0" style={{ background: t.insetBg, border: `1px solid ${t.inputBorder}` }}>
                  ...
                  <p style={{ fontSize: 10.5, color: '#4e342e', margin: 0, fontWeight: 700, whiteSpace: 'nowrap' }}>{age.years}y {age.months}m</p>
                </div>
              )}
            </div>
          </Field>
        ...
        {!isView && (
          <div className="flex justify-end mb-4">
            <button type="button"
              ...
              style={{ background: t.insetBg, color: '#4e342e', border: `1px solid ${t.inputBorder}`, cursor: 'pointer' }}>
        ...
        {!isView && (
          <>
            ...
            <button type="button" onClick={handlePreview} disabled={saving}
              ...
              style={{ background: t.insetBg, color: '#4e342e', border: `1px solid ${t.inputBorder}`, cursor: saving ? 'not-allowed' : 'pointer' }}>
