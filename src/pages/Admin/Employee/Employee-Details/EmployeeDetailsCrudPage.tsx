  ...
  const content = (
    <>
      <div
        ...
        style={{
          height: 120,
          background: isImage ? t.insetBg : url ? 'var(--master-btn-primary-gradient, #4e342e)' : t.insetBg,
        }}
      >
        {isImage && url ? (
          ...
          <Field t={t} label="Salary" required error={errorFor('salary')} fieldRef={setFieldRef('salary') as React.Ref<HTMLDivElement>}>
            <div className={`flex items-center gap-2 ${fieldClass}`} style={{ padding: '0 12px' }}>
              ...
              {compactINR(form.salary) && (
                <span style={{ color: '#4e342e', fontWeight: 700, fontSize: 10, flexShrink: 0, whiteSpace: 'nowrap' }}>{compactINR(form.salary)}</span>
              )}
            </div>
          </Field>
