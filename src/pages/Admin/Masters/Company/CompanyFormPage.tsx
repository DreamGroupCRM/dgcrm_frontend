...
// ─────────────────────────────────────────────────────────────────────────────
// CRITICAL: Field is defined at MODULE LEVEL — NOT inside CompanyFormPage.
//
// If Field were defined inside the parent function, React would create a
// brand-new component type on every state change, causing it to unmount and
// remount on every keystroke — which is why the cursor was disappearing.
//
// Keeping Field here (module scope) gives it a stable identity across renders.
// ─────────────────────────────────────────────────────────────────────────────
...
const empty: FormState = {
    ...
    pincode: '', pan: '', gst: '', logo_url: '', company_code: '',
};
...
const CompanyFormPage: React.FC<Props> = ({ mode }) => {
            ...
            try {
                ...
                if (res.success && res.data) {
                    ...
                    setForm({
                        ...
                        pan: d.pan ?? '',
                        gst: d.gst ?? '',
                        logo_url: (d.logo_url && d.logo_url !== 'string') ? d.logo_url : '',
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            if (form.company_code.trim()) payload.company_code = form.company_code.trim();
            ...
            if (res.success) {
                toast.success(isEdit ? 'Company Updated Successfully' : 'Company Created Successfully', { autoClose: 1000 });
                ...
                <div className="flex items-center gap-3">
                    <button onClick={handleSubmit} disabled={!isMandatoryValid || saving}
                        ...
                        style={{
                            ...
                            border: 'none', cursor: !isMandatoryValid || saving ? 'not-allowed' : 'pointer',
