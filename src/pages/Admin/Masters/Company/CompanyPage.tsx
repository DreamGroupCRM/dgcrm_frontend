...
const CompanyPage: React.FC = () => {
  ...
  const exportCSV = () => {
    ...
    const rows    = filtered.map((c) => [
      c.id, `"${c.name}"`, c.email, c.phone, formatDate(c.created_at),
    ]);
    ...
    <div style={{ fontFamily: t.fontFamily }}>
      ...
      <div className="flex flex-wrap items-center gap-3 mb-4">
        ...
        <div className="flex items-center gap-2 ml-auto flex-wrap">
          <button
            onClick={() => navigate(`${ROUTES.ADMIN.COMPANY}/add`)}
        ...
        <div className="flex flex-wrap items-center justify-between gap-3 px-4 py-3" style={{ borderTop: `1px solid ${t.divider}` }}>
          ...
          <div className="flex items-center gap-1">
            <button onClick={() => setPage((p) => Math.max(1, p - 1))} disabled={page === 1}
              style={{ padding: '4px 10px', borderRadius: 8, border: `1px solid ${t.surfaceBorder}`, background: t.btnSecondaryBg, color: page === 1 ? t.textMuted : t.textPrimary, cursor: page === 1 ? 'not-allowed' : 'pointer', fontSize: 13 }}>
              Prev
            </button>
            {pageBtns().map((pg) => (
            ...
            <button onClick={() => setPage((p) => Math.min(totalPages, p + 1))} disabled={page >= totalPages}
              style={{ padding: '4px 10px', borderRadius: 8, border: `1px solid ${t.surfaceBorder}`, background: t.btnSecondaryBg, color: page >= totalPages ? t.textMuted : t.textPrimary, cursor: page >= totalPages ? 'not-allowed' : 'pointer', fontSize: 13 }}>
              Next
            </button>
          </div>
        </div>
